...
test("public URL validation rejects SSRF targets and embedded credentials", async () => {
  ...
  await assert.rejects(() => assertPublicHttpUrl("https://product.example.test", async () => ["10.0.0.4"]));
});

test("public URL validation accepts a resolved public HTTP target", async () => {
  const value = await assertPublicHttpUrl("https://product.example.test/path", async () => ["203.0.113.10"]);
